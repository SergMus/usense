...
/**
 * Expects 0 Components to be found by its selector, e.g. `app-example`.
 */
export function expectNoComponent<T>(fixture: ComponentFixture<T>, selector: string): void {
	expect(findComponents(fixture, selector).length).toBe(0);
}

/**
 * Expects 0 Components to be found by theirs selectors, e.g. `app-example`, `app-another-example`.
 */
export function expectNoComponents<T>(fixture: ComponentFixture<T>, ...selectors: string[]): void {
	for (let selector of selectors) {
		expectNoComponent(fixture, selector);
	}
}

/**
 * Expects Component to be found by its selector, e.g. `app-example`.
 */
export function expectComponent<T>(fixture: ComponentFixture<T>, selector: string): void {
	expect(findComponent(fixture, selector)).toBeTruthy();
}

/**
 * Whenever you need to run change detection for a component using the OnPush change detection strategy, you can call this function
 */
export async function runOnPushChangeDetection(fixture: ComponentFixture<any>): Promise<any> {
